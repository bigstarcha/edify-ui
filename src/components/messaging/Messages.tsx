...
const sampleMessages = [
    ...
    { isMe: false, text: 'It was good! I took my dog for a walk at the city park today. Loved enjoying the scenery. And you?' },
    { isMe: true, text: 'Mine was pretty good! I had a good time hangout with my friends after church. Loved to spend some quality time with them.' },
    { isMe: false, text: 'Hey, are you down to get dinner sometime this week? LOL' },
    { isMe: true, text: 'I am so down! Where are you thinkin?' },
    { isMe: false, text: 'Wanna try this new spot 15 minutes away from my place? Its a new Italian place and I heard its the new deal' },
    { isMe: true, text: 'Count me in!!' },
    { isMe: false, text: 'Cool, see you there this Thursday at 7 pm?' },
    { isMe: true, text: 'Roger that!' },
    { isMe: false, text: 'Oh, btw. I forgot to mention, can I bring my other friend along?' },
...
function Messages() {
    ...
    return (
        <div className="flex flex-col w-full h-9/10 mt-[20px]">
            ...
            <div className="flex flex-row px-7 items-center shadow-sm pb-[20px]">
                <Image src={sampleperson} alt="Sample Bio" width={80} height={80} style={{ borderRadius: '40px' }} />
                <h1 className="ml-[25px] text-2xl font-semibold">Brandon Lake</h1>
            </div>
            <div className="flex flex-col w-full h-full px-7 overflow-y-scroll">
                <div className="mt-[20px]"></div>
                {sampleMessages.map((message, index) => (
                    <Message key={index} isMe={message.isMe} text={message.text} />
                ))}
            </div>
        </div>
    );
