...
function Message({ isMe, text }: { isMe: boolean, text: string }) {
    ...
    return (
        <div className={`flex flex-row mb-[10px] ${isMe ? 'justify-end' : 'justify-start'}`}>
            <div className={`py-2 px-4 ${isMe ? 'bg-linear-to-r from-primary-400 to-primary-500 text-white' : 'bg-gray-200 text-black'} rounded-2xl w-max max-w-[50%]`}>
